...
export class BpmnCanvasValidator {
  validate(modeler: any): ValidationResult {
    ...
    // ── Rule 6: No unsupported element types ────────────────────────
    for (const el of shapes) {
      if (!SUPPORTED_TYPES.has(el.type)) {
        errors.push({
          ...
          message: `Element type "${el.type}" is not supported by the MVP engine. Remove it.`,
          severity: 'error',
        });
      }
    }
    ...
    for (const shape of shapes) {
      if (shape.type === 'bpmn:StartEvent') continue; // Start checked separately
      if (shape.type === 'bpmn:EndEvent') continue;   // End checked separately
    ...
    // ── Rule 8: Start Event connectivity ───────────────────────────
    ...
    // ── Rule 5: Service Tasks must have a name ─────────────────────
    for (const task of serviceTasks) {
      ...
      if (!name) {
        errors.push({
          ...
          message: `A Service Task (${task.id}) has no name. The name must match a registered handler (e.g. "ContainerValidationHandler").`,
          severity: 'error',
        });
      }
    }

    // ── Rule 4 + 7: Exclusive Gateway conditions ────────────────────
    for (const gw of gateways) {
      const outgoing: any[] = gw.outgoing || [];

      // Rule 7: must have ≥ 2 outgoing
      if (outgoing.length < 2) {
        warnings.push({
          elementId: gw.id,
          elementName: gw.businessObject?.name || gw.id,
          rule: 'GATEWAY_SINGLE_OUTGOING',
          message: `Gateway "${gw.businessObject?.name || gw.id}" has only ${outgoing.length} outgoing flow(s). An Exclusive Gateway typically branches into ≥ 2 paths.`,
          severity: 'warning',
        });
      }

      // Rule 4: all outgoing flows must have conditionExpression
      for (const flow of outgoing) {
        ...
        if (!condition) {
          errors.push({
            ...
            message: `Sequence flow from gateway "${gw.businessObject?.name || gw.id}" is missing a condition expression. The engine cannot route without it.`,
            severity: 'error',
          });
        }
      }
